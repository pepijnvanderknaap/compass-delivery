-- Safe cleanup of duplicate locations
-- This will keep the OLDER location (created first) and migrate all data to it
...
BEGIN
  -- Step 1: Migrate all orders from duplicates to the locations we're keeping

  -- Atlasian
  UPDATE orders SET location_id = keep_atlasian WHERE location_id = remove_atlasian;

  -- JAA Training
  UPDATE orders SET location_id = keep_jaa WHERE location_id = remove_jaa;

  -- SnapChat 119
  UPDATE orders SET location_id = keep_snap119 WHERE location_id = remove_snap119;

  -- SnapChat 165
  UPDATE orders SET location_id = keep_snap165 WHERE location_id = remove_snap165;

  -- Snowflake
  UPDATE orders SET location_id = keep_snowflake WHERE location_id = remove_snowflake;

  -- Symphonys
  UPDATE orders SET location_id = keep_symphonys WHERE location_id = remove_symphonys;

  RAISE NOTICE 'Orders migrated to older location IDs';
  ...
  UPDATE location_settings SET location_id = keep_atlasian WHERE location_id = remove_atlasian;
  UPDATE location_settings SET location_id = keep_jaa WHERE location_id = remove_jaa;
  UPDATE location_settings SET location_id = keep_snap119 WHERE location_id = remove_snap119;
  UPDATE location_settings SET location_id = keep_snap165 WHERE location_id = remove_snap165;
  UPDATE location_settings SET location_id = keep_snowflake WHERE location_id = remove_snowflake;
  UPDATE location_settings SET location_id = keep_symphonys WHERE location_id = remove_symphonys;

  RAISE NOTICE 'Location settings migrated';
