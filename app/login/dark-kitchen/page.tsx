...
export default function DarkKitchenLoginPage() {
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (data.user) {
        ...
        // Verify user has access to Dark Kitchen (admin or kitchen roles)
        if (profile && (profile.role === 'admin' || profile.role === 'kitchen')) {
          router.push('/dark-kitchen');
          router.refresh();
        } else {
          setError('You do not have access to the Dark Kitchen area');
